  ...
  where(e) {
    ...
    if (t.length === 1) {
      ...
      for (const i of n) {
        let l = s[i];
        if (i === "=")
          return this.whereCause.keyRange = g.only(l), this;
        const o = !i.endsWith("="), u = o ? i : i.slice(0, 1);
        let p = this.whereCause.keyRange;
        if (p) {
          let { lower: h, upper: d, lowerOpen: w, upperOpen: k } = p;
          return h ? (d = l, k = o) : (h = l, w = o), h >= d && f("lower must to less than upper value"), this.whereCause.keyRange = g.bound(h, d, w, k), this;
        }
        this.whereCause.keyRange = g[R[u]](l, o);
      }
    }
    return this;
  }
  ...
  create(e) {
    ...
    if (!(t.length <= 1))
      for (let l = 1; l < t.length; l++) {
        const o = t[l].trim(), u = o.startsWith("&"), p = u ? o.slice(1) : o;
  ...
  find(e = {}) {
    ...
    return new Promise((i, l) => {
      ...
      p.onsuccess = (h) => {
        ...
        if (d) {
          if (u.push(d.value), r && u.length >= r)
            return i(u), u;
          d.continue();
        } else
          i(u);
  ...
  remove(e = {}) {
    ...
    return new Promise((i, l) => {
      ...
      p.onsuccess = (h) => {
        const d = h.target.result;
        if (d) {
          if (u.push(d.value), d.delete(), r && u.length >= r) {
            i(u);
            return;
          }
          d.continue();
  ...
  open(e) {
    return this.close(), this._stores(e), new Promise((t, r) => {
      this.request = C.open(this._name, this._version), this.request.onupgradeneeded = (s) => {
        this._idb = s.target.result, this._addStores(e), this.upgradeFunc && this.upgradeFunc(s), console.log("onupgradeneeded");
      }, this.request.onsuccess = (s) => {
        this._idb = s.target.result, this._isOpen = !0, t(s);
